...
export function useRealtime({
  ...
  useEffect(() => {
    ...
    const name = channelName || `realtime-${subscriptions.map((s) => s.table).join("-")}-${Date.now()}`;

    console.log("[Realtime] Setting up channel:", name);
    ...
    subscriptions.forEach((subscription) => {
      ...
      const handleChange = (payload: { eventType: "INSERT" | "UPDATE" | "DELETE"; new: Record<string, unknown>; old: Record<string, unknown> }) => {
